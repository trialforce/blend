...
var nicEditorConfig = bkClass.extend({
    buttons: {
        ...
        'italic': {name: __('Itálico'), command: 'Italic', tags: ['EM', 'I'], css: {'font-style': 'italic'}, key: 'i'},
        'underline': {name: __('Sublinhado'), command: 'Underline', tags: ['U'], css: {'text-decoration': 'underline'}, key: 'u'},
        'left': {name: __('Alinhar a esquerda'), command: 'justifyleft', noActive: true},
        'center': {name: __('Centralizar'), command: 'justifycenter', noActive: true},
        'right': {name: __('Alinhar a direita'), command: 'justifyright', noActive: true},
        'justify': {name: __('Justificar'), command: 'justifyfull', noActive: true},
        'ol': {name: __('Lista númerica'), command: 'insertorderedlist', tags: ['OL']},
        'ul': {name: __('Lista simples'), command: 'insertunorderedlist', tags: ['UL']},
        'subscript': {name: __('Sub'), command: 'subscript', tags: ['SUB']},
        'superscript': {name: __('Super'), command: 'superscript', tags: ['SUP']},
        'strikethrough': {name: __('Traçado'), command: 'strikeThrough', css: {'text-decoration': 'line-through'}},
        'removeformat': {name: __('Remover formataçao'), command: 'removeformat', noActive: true},
        'indent': {name: __('Identar'), command: 'indent', noActive: true},
        'outdent': {name: __('Remover identação'), command: 'outdent', noActive: true},
        'hr': {name: __('Linha'), command: 'insertHorizontalRule', noActive: true}
    },
    iconsPath: '../nicEditorIcons.gif',
    buttonList: ['bold', 'italic', 'underline', 'left', 'center', 'right', 'justify', 'fontSize', 'fontFormat', 'ol', 'ul', 'link', 'unlink', 'forecolor', 'bgcolor', 'indent', 'outdent', 'youtube', 'upload'],
    iconList: {"bgcolor": 1, "forecolor": 2, "bold": 3, "center": 4, "hr": 5, "indent": 6, "italic": 7, "justify": 8, "left": 9, "ol": 10, "outdent": 11, "removeformat": 12, "right": 13, "save": 24, "strikethrough": 15, "subscript": 16, "superscript": 17, "ul": 18, "underline": 19, "image": 20, "link": 21, "unlink": 22, "close": 23, "arrow": 25,  "youtube": 29, "upload": 28}
});
;
...
var nicEditorInstance = bkClass.extend({isSelected: false, construct: function (G, D, C) {
        ...
        if (H || this.options.hasPanel) {
            ...
            var A = new bkElement("DIV").setStyle({width: (newX - 8) + "px", minHeight: newY + "px"}).addClass("main").appendTo(this.editorContain);
...
var nicEditorButton = bkClass.extend({construct: function (D, A, C, B) {
        ...
        this.contain.setStyle({opacity: 0.}).removeClass("buttonEnabled");
        this.updateState()
...
var nicEditorAdvancedButton = nicEditorButton.extend({init: function () {
        ...
        for (itm in C) {
            ...
            if (A == "title") {
                ...
                switch (A) {
                    case"text":
                        this.inputs[itm] = new bkElement("input").setAttributes({id: itm, value: F, type: "text"}).setStyle(D.style).appendTo(B);
                        break;
                    case"select":
        ...
        new bkElement("input").setAttributes({type: "submit"}).appendTo(this.form);
...
var nicSelectOptions = {
    buttons: {
        'fontSize': {name: __('Escolha o tamanho'), type: 'nicEditorFontSizeSelect', command: 'fontsize'},
        'fontFamily': {name: __('Escolha a fonte'), type: 'nicEditorFontFamilySelect', command: 'fontname'},
        'fontFormat': {name: __('Escolha o formato'), type: 'nicEditorFontFormatSelect', command: 'formatBlock'}
    }
};
...
var nicEditorSelect = bkClass.extend({construct: function (D, A, C, B) {
        ...
        this.isDisabled = true;
        this.close();
        this.contain.setStyle({opacity: 0});
        this.contain.setStyle({display: 'none'});
    }, enable: function (A) {
        this.isDisabled = false;
        this.close();
        this.contain.setStyle({opacity: 1})
        this.contain.setStyle({display: 'block'});
...
var nicEditorFontFormatSelect = nicEditorSelect.extend({sel: {p: "Parágrafo", pre: "Pre", h6: "Título&nbsp;6", h5: "Título&nbsp;5", h4: "Título&nbsp;4", h3: "Título&nbsp;3", h2: "Título&nbsp;2", h1: "Título&nbsp;1"}, init: function () {
...
var nicLinkButton = nicEditorAdvancedButton.extend({addPane: function () {
        ...
        this.addForm({"": {type: "title", txt: "Adicionar/Editar link"}, href: {type: "text", txt: "URL", value: "http://", style: {width: "150px"}}, title: {type: "text", txt: "Título"}, target: {type: "select", txt: "Abrir em", options: {"": "Janela atual", _blank: "Nova janela"}, style: {width: "100px"}}}, this.ln)
    }, submit: function (C)
    {
        ...
        if (A === "http://" || A === "")
        {
            alert("É necessário entrar com uma url");
            return false
        }
        this.removePane();
...
var nicColorOptions = {
    buttons: {
        'forecolor': {name: __('Trocar cor do texto'), type: 'nicEditorColorButton', noClose: true},
        'bgcolor': {name: __('Trocar cor de fundo'), type: 'nicEditorBgColorButton', noClose: true}
    }
};
...
/*Blend media plugin*/
var nicMediaButton = nicEditorAdvancedButton.extend({addPane: function ()
    {
        passThis = this;
        e('addmedia');
    }, submit: function (C)
    {
        $('.nicEdit-main').focus();
        var isImage = $('#isImage').val() === 'true';
        var html = '';

        var selectedFile = $('#selectedMedia');
        var alt = $('#alt');
        ...
        if (isImage)
        {
            ...
            var href = selectedFile.attr('href');

            mediaReturnFileName = '';

            var url = getCurrentPage() + '/resizeImage/?href=' + href + '&width=' + width + '&height=' + height;
            ...
            if (mediaReturnFileName !== '')
            {
                html = '<a href="' + href + '" target="_blank" ><img alt="' + alt.val() + '" src="' + mediaReturnFileName + '" width ="' + mediaReturnWidth + '" height="' + mediaReturnHeight + '"  /></a>';
            }
            else //without resize, a error
            {
                html = '<a href="' + href + '" target="_blank" ><img alt="' + alt.val() + '" src="' + href + '" width ="' + width + '" height="' + height + '"  /></a>';
            }
        }
        else
        {
            html = '<a href="' + selectedFile.attr('href') + '" target="_blank" >' + alt.val() + '</a>'
        }
...
var nicMediaOptions = {
    buttons: {
        'media': {name: 'Adicionar media', type: 'nicMediaButton', tags: ['IMG']}
    }

};

var passThis = null;
var mediaReturnFileName = null;
var mediaReturnWidth = null;
var mediaReturnHeight = null;
...
var nicYouTubeButton = nicEditorAdvancedButton.extend({
    ...
    addPane: function () {
        this.addForm({
            '': {type: 'title', txt: 'Link do youtube'},
    ...
    submit: function (e) {
        ...
        //resolve problema em url do youtube
        //https://www.youtube.com/watch?feature=player_embedded&v=qm-Bkx8z-1Q
        //https://www.youtube.com/watch?&v=qm-Bkx8z-1Q
        if (code.indexOf('feature=player_embedded&') > 0)
        {
            code = code.replace('feature=player_embedded&', '');
        }
        ...
        var youTubeCode = '<div class="stretch-video"><iframe width="' + width + '" height="' + height + '" src="' + code + '" frameborder="0" allowfullscreen></iframe></div>';
...
var nicUploadButton = nicEditorAdvancedButton.extend({
    nicURI: $('base').attr('href') + '/nicupload.php',
    errorText: 'Falha ao enviar imagem',
    addPane: function ()
    {
        if (typeof window.FormData === "undefined")
        {
            return this.onError("Envio de imagem não suportado pelo seu navegador.");
        }
        ...
        new bkElement('div')
                ...
                .setContent('Selecione uma imagem')
                .appendTo(container);
        ...
        this.myBr = new bkElement('br').appendTo(container);
        this.myBr = new bkElement('br').appendTo(container);

        this.myLabel = new bkElement('label')
                .setStyle({fontSize: '14px', fontWeight: 'bold', paddingBottom: '5px', 'marginRight': '10px'})
                .setContent('Largura:')
                .appendTo(container);

        this.myWidth = new bkElement("input")
                .setAttributes({id: 'myWidth', value: '', type: "text"})
                .setStyle({width: '50px'})
                .appendTo(container);

        this.myLabel = new bkElement('label')
                .setStyle({fontSize: '14px', paddingBottom: '5px', 'marginLeft': '10px'})
                .setContent('px')
                .appendTo(container);
    ...
    {
        this.removePane();
        alert(msg || "Falha ao enviar imagem");
    },
    uploadFile: function ()
    {
        ...
        {
            this.onError("Somente imagens pode ser enviadas");
            return;
        }
        ...
        fd.append("width", this.myWidth.value);
